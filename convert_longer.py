...
def convert_mp4_to_gif_longer(input_path, output_path):
    try:
        # OpenCV로 비디오 로드
        ...
        if frames:
            ...
            print(f"긴 재생시간 GIF 변환 완료: {output_path}")
            return True
        else:
...
if os.path.exists(input_file):
    success = convert_mp4_to_gif_longer(input_file, output_file)
    if success:
        print("긴 재생시간 GIF 파일이 성공적으로 생성되었습니다!")
    else:
        print("변환에 실패했습니다.")
else:
